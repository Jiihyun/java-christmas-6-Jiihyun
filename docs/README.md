...
- [] 메뉴마다 Menu 가격과 수량의 곱 구해서 더해주기
- [] `0,000,000원\n` 형식으로 출력
- [] 만원 미만일시 할인x -> 만원부터 할인 가능

### 4️⃣ 샴페인 증정 여부 확인

- [] 할인 전 총주문 금액이 12만원 이상이면 증정
- []  `없음` or `샴페인 1개` 출력

### 5️⃣ 각 이벤트마다의 할인 및 혜택 금액 계산

* `크리스마스 디데이 할인`
    - [] 이벤트 기간 검증 (~2023.12.25)
    - [] 1,000원으로 시작하여 크리스마스가 다가올수록 날마다 할인 금액이 100원씩 증가시키기
      (e.g. 시작일인 12월 1일에 1,000원, 2일에 1,100원, ..., 25일엔 3,400원 할인)
    - [] 출력: `-0,000원` 형식으로
...
* `특별 할인`
    - [] 날짜 검증 (매 주 일요일_3,10,17,24,31 + 25일 당일)
    - [] 1,000원 할인
    - [] 출력: `-0,000원` 형식으로

* `증정 이벤트 혜택`
    - [] 25,000원 할인 (할인 전 총주문 금액이 12만원 이상이면 증정)
    - [] 출력: `-0,000원` 형식 or `없음`
